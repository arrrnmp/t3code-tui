/**
 * Vendored from OpenCode (`packages/opencode/src/plugin/xai.ts`) at
 * opencode/opencode@d870e22 — MIT, (c) 2025 opencode.
 *
 * Byte-identical to upstream apart from the three import lines below,
 * which are rewired to the local `./shims.js` (same values/signatures;
 * see `shims.ts`). In particular the `referrer: "opencode"` device-code
 * parameter is kept as-is: it is part of the flow, not our branding.
 * Full provenance + license in `THIRD_PARTY_NOTICES`.
 */

/**
 * Vendored from OpenCode (`packages/opencode/src/plugin/openai/codex.ts`) at
 * opencode/opencode@d870e22 — MIT, (c) 2025 opencode.
 *
 * Two mechanical adaptations, both documented in `THIRD_PARTY_NOTICES`:
 * 1. Import lines rewired to the local `./shims.js` (same
 *    values/signatures; see `shims.ts`).
 * 2. The experimental WebSocket transport branch removed
 *    (`./ws-pool` import, `experimentalWebSockets` option, the
 *    `websocketFetch` creation/passthrough, the `dispose`/`event` hooks
 *    that only managed its lifecycle, and the `TITLE_HEADER` hack in
 *    `chat.headers`). HTTP/SSE stays, exactly as upstream's default path.
 * Everything else is byte-identical to upstream.
 */
import type { Hooks, PluginInput } from "./shims.js";
import { InstallationVersion } from "./shims.js";
import { OAUTH_DUMMY_KEY } from "./shims.js";
import os from "os";
import { setTimeout as sleep } from "node:timers/promises";
import { createServer } from "http";
import { OauthCallbackPage } from "./shims.js";
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            const authWithAccount = currentAuth as typeof currentAuth & { accountId?: string | undefined };
